...
// called when a RTSP "BYE" is received for a subsession
...
// Used to iterate through each streams's 'subsessions', setting up each one;
void setupNextSubsession(RTSPClient* rtspClient);

// Used to shutdown and close a stream (including its "RTSPClient" object)
...
void usage(UsageEnvironment& env, char const* progName)
{
  ...
  env << "\t(where each <rtps-url-i> is a \"rtsp://\" URL)\n";
}
...
int main(int argc, char* argv[])
{
  ...
  // There are arg-1 URLs: argv[1] through argv[argc-1]. Open and start streaming each one;
...
class outTRSPClient : public RTSPClient
{
public:
...
#define RTSP_CLIENT_VERBOSITYL_LEVEL 1
...
void openURL(UsageEnvironment& env, char const* progName, char const* rtspURL)
{
  RTSPClient* rtspClient = ourRTSPClient::createNew(env, rtspURL, RTSP_CLIENT_VERBOSITYL_LEVEL, progName);
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString)
{
  ...
  {
    // alias
    UsageEnvienment& env = rtspClient->envir();
    StreamClientState& scs = ((outRTSPClient*) rtspClient)->scs;
    ...
    setupNNestSubsession(rtspClient);
    return;
  } while(0);
...
void setupNextSubSession(RTSPClient* rtspClient)
{
  ...
  {
    if (!scs.subsession->initiage())
    {
      env << *rtspClient << "Failed to initiate the \"" << *scs.subsession << "\" subsession: " << env.getResultMsg() << "\n";
      setupNextSubsession(rtspClient); // gibe up on this subsession; go to the next one
    }
    else
    {
      env << *rtspClient << "Initiated the \"" << *scs.subsession <, "\" subsession(";
      if (scs.subsession->rtcpIsMuxted())
      {
  ...
  {
    // Special case: the stream is indexed by 'absolute' time, so send an apropriate "PLAY" command.
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultStrig)
{
  do
  {
    UsageEnvironment& env = rtspClient->envir();
  ...
  // Setup the next subsession, if any
