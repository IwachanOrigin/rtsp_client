...
void AudioRenderer::audioCallback(void* userdata, Uint8* stream, int len)
{
  ...
  while (len > 0)
  {
    ...
    if (audioBufIndex >= audioBufSize)
    {
      ...
      audioSize = audioDecodeFrame(videoState, audioBuf, audioArrayBufSize, pts);
      ...
      {
        audioSize = client::syncAudio(videoState, (int16_t *)audioBuf, audioSize);
...
int AudioRenderer::audioDecodeFrame(VideoState* vs, uint8_t* audio_buf, int bufSize, double& pts_ptr)
{
  ...
  for (;;)
  {
    while (audioPktSize > 0)
    {
      ...
      {
        // audio resampling
        dataSize = client::audioResampling(
          vs
          , avFrame
  ...
  , uint8_t* out_buf)
{
  ...
  {
    client::audioReleasePointer(arState);
    return -1;
  }
...
int AudioRenderer::syncAudio(VideoState* vs, short* samples, int& samplesSize)
{
  auto& audioCodecCtx = vs->audioCodecCtx();
